...
using Dto.ReceiptDto;
using ECom.Services.Cart.Data;
using ECom.Services.Cart.Models;
using ECom.Services.Cart.Utility;
using Messages;
...
namespace ECom.Services.Billing.Handler
{
        ...
        IHandleMessages<CreateReceipt>
    {
        ...
        public async Task Handle(GetReceiptByStatus message, IMessageHandlerContext context)
        {
            ...
            try
            {
                List<Cart> receipts = DataAccess.Ins.DB.Carts.Where(u => u.Status.Equals(message.Status)).OrderBy(u => u.Id).ToList();
