  ...
  const columns: ColumnsType<IImportDetail> = [
    ...
    },
    {
      ...
      render: (text: string, record: IImportDetail) => <Space direction='horizontal'>
        <Image className='imgborder' width={100} height={120} alt="example" src={record.product?.image[0]} />
    ...
    },
    {
      title: 'Phân loại',
      key: 'size-color',
      render: (text: string, record: IImportDetail) => <p>{record.product?.productItems[0]?.size + "/" + record.product?.productItems[0]?.color}</p>,
    },
    {
      title: 'Giá nhập',
      dataIndex: 'price',
    ...
    },
    {
      ...
      key: 'sold',
      render: (text: number, record: IImportDetail) => <p>{formatNumberWithComma(record.totalCost)}</p>,
    },
  ];
